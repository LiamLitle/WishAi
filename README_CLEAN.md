...
|---|---|---|
| 📊 | **Dashboard natif** | Fenêtre locale, ~60 Mo de RAM |
| 🛡️ | **Protection auto** | VRAM, RAM, temp — jamais de crash |
| 📚 | **Bibliothèque** | 135 curatés + 100k+ accessibles |
| 🔋 | **Accumulation** | Gros modèles sur petites VRAM |
| 🔄 | **Early stopping** | S'arrête tout seul à convergence |
| 🧠 | **BPE from scratch** | 3× plus de contexte que char-level |
...
## 🛡️ Protections automatiques — la feature que personne n'a
...
- **`DATASETS.md`** : Liste complète des 135 datasets disponibles, organisés par catégorie.
- **`PARAMETRES.md`** : Guide expert expliquant chaque paramètre d'entraînement (batch_size, learning_rate, dropout, etc.).

---

## ⚙️ Architecture interne — comment les composants communiquent
...
| `/api/pwc` | GET | Proxy Papers with Code (contourne CORS) |
...
Ouvre la bibliothèque (bouton dans le dashboard ou `python src/telecharger.py`), active la source **HuggingFace**, tape un mot-clé. Tu as accès aux 150 000+ datasets du Hub en direct.
...
❌ Vendre ce logiciel ou un produit basé directement sur son code : interdit
❌ Republier ce logiciel en le faisant passer pour ta propre création : interdit
❌ Se proclamer auteur ou créateur de ce projet : interdit

→ [LICENSE](LICENSE) · Pour une autorisation commerciale : ouvre une Issue GitHub.

---

## Contribuer

🐛 Bug → Issue GitHub    💡 Idée → Pull Request    ⭐ Utile → Une étoile, ça fait plaisir
