...
class ThreadingHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def handle_error(self, request, client_address):
        # Ignore les deconnexions browser normales (WinError 10053, BrokenPipe, etc.)
        exc = sys.exc_info()[1]
...
class WishAIHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_sse(self):
        ...
        def _get_monitor_url():
            port_file = os.path.join(ROOT, "system", "monitor_port.json")
            try:
        ...
        try:
            while True:
                ...
                # -- session --
                try:
                    session_file = os.path.join(ROOT, "system", "session.json")
...
def main():
    ...
    with open(os.path.join(ROOT, "system", "dashboard_url.json"), "w", encoding="utf-8") as _f:
