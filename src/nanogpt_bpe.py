...
def choisir_donnees():
    """
    Cherche les fichiers de données disponibles et demande lequel utiliser
    si plusieurs langues sont présentes.
    """
    global DATA_FILE
    langues_dispo = []
    for langue in ["fr", "en", "multi"]:
        chemin = os.path.join("data", langue, "data.txt")
        if os.path.exists(chemin):
            taille = os.path.getsize(chemin) / 1_000_000
            langues_dispo.append((langue, chemin, taille))

    if len(langues_dispo) == 0:
        if os.path.exists(DATA_FILE):
            taille = os.path.getsize(DATA_FILE) / 1_000_000
            print(f"  Données : {DATA_FILE} ({taille:.1f} Mo)")
            return
    ...
    # Plusieurs langues disponibles → demander laquelle utiliser
    print("\n" + "="*62)
    ...
    try:
        idx = int(choix) - 1
        if 0 <= idx < len(langues_dispo):
            langue, chemin, taille = langues_dispo[idx]
            DATA_FILE = chemin
            return
    except ValueError:
        pass
    # Si mauvais choix → prend le premier par défaut
    langue, chemin, taille = langues_dispo[0]
    DATA_FILE = chemin
...
#
_data_dir  = os.path.dirname(DATA_FILE) or "."
...
with open(os.path.join("model", "active.json"), "w", encoding="utf-8") as _f:
...
if not _QUICK_MODE and os.path.exists(CHECKPOINT_FILE):
    ...
    # Proposer de partir d'un modèle existant (fine-tuning)
    modeles_dispo = sorted(glob.glob(os.path.join("model", "*", "modele.pt")))
    if modeles_dispo:
        print("\n" + "="*62)
