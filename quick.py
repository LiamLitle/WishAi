...
#
# Ce script :
#   1. Crée un texte de démonstration si aucune donnée n'existe
...
def creer_texte_demo():
    """Génère ~400Ko de texte de démonstration et le sauvegarde dans data/data.txt."""
    import random
    random.seed(42)
    ...
    print(f"  ✅ Données de démo créées : data/data.txt ({taille_ko} Ko)")
...
if not donnees_existent():
    print("  Aucune donnée trouvée — création d'un texte de démo...")
    creer_texte_demo()
    print("  💡 Pour de meilleurs résultats : remplace data/data.txt par ton propre texte.")
else:
...
_ctrl = os.path.join(ROOT, "control.json")
_tmp  = _ctrl + ".tmp"
with open(_tmp, "w", encoding="utf-8") as _f:
    json.dump({"commande": "run", "timestamp": time.time()}, _f)
